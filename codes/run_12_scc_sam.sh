...
SUMMARY="${SUMMARY:-codes/results_scc_sam_12runs_live.txt}"
LOG_DIR="${LOG_DIR:-codes/scc_sam_12runs_live_logs}"
N_THREADS="${N_THREADS:-$(nproc)}"
mkdir -p "${LOG_DIR}"

: > "${SUMMARY}"
{
    echo "Recomputed SCC/SAM for 12 result folders"
...
run_metric() {
    ...
    if [[ ! -d "${gt}" ]]; then
        echo "Missing GT directory for ${id}: ${gt}" | tee -a "${SUMMARY}"
        exit 1
    fi
    if [[ ! -d "${gen}" ]]; then
        echo "Missing generated directory for ${id}: ${gen}" | tee -a "${SUMMARY}"
        exit 1
    fi
    ...
    "${PYTHON_BIN}" -u codes/calculate_SCC_SAM.py \
        --dataset "${dataset}" \
        --scale "${scale}" \
        --n_threads "${N_THREADS}" \
        --folder_GT "${gt}" \
        --folder_Gen "${gen}" 2>&1 | tee "${log}"

    local avg
    avg="$(grep 'Average:' "${log}" | tail -n 1)"
    echo "${avg}" | tee -a "${SUMMARY}"
    echo >> "${SUMMARY}"
}
