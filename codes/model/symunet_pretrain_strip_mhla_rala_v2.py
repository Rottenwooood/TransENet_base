...
import sys
sys.path.append('..')
from MHLA_RALA_v2 import MHLA_Normed_Torch_Dynamic_RALA
...
def make_model(args, parent=False):
    return SymUNet_Pretrain_Strip_MHLA_RALA_V2(args)
...
class MAB(nn.Module):
    def __init__(self, dim, num_head=2, kernel_sizes=[7, 11], dilations=[1, 1]):
        ...
        print(f"[MAB] 使用 MHLA2D_RALA_V2 (num_head={num_head})")
...
#@ARCH_REGISTRY.register("symunet_pretrain_strip_mhla_rala_v2")
class SymUNet_Pretrain_Strip_MHLA_RALA_V2(nn.Module):
    """
    symunet_pretrain_strip_mhla_rala_v2: MAB使用RALA_v2版本的MHLA
    - 使用 LAB (Local Aggregation Block)
    - 仅使用 MAB2 (dilations=[5,3])
    - 使用 MSConvStar
    - Middle Blk使用StripAttention (k1=1, k2=47)
    - MAB使用RALA_v2版本的MHLA (MHLA_RALA_v2模块)
    """
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_Strip_MHLA_RALA_V2, self).__init__()
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_Strip_MHLA_RALA_V2(args)
    model.eval()
