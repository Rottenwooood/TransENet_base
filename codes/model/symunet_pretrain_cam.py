...
def make_model(args, parent=False):
    return symunet_pretrain_cam(args)
...
class symunet_pretrain_cam(nn.Module):
    """
    S1_Trans_NoMAB1_CA_Middle: 去掉MAB1 + CA Middle的变体
    - 使用 LAB (Local Aggregation Block)
    - 仅使用 MAB2 (dilations=[5,3])
    - 使用 MSConvStar
    - Middle Blk使用ChannelAttention替代self-attention
    """
    def __init__(self, args, conv=common.default_conv):
        super(symunet_pretrain_cam, self).__init__()
