...
def make_model(args, parent=False):
    return SymUNet_Pretrain_Strip_CAM_Parallel_Add(args)
...
#@ARCH_REGISTRY.register("symunet_pretrain_strip_cam_parallel_add")
class SymUNet_Pretrain_Strip_CAM_Parallel_Add(nn.Module):
    """
    symunet_pretrain_strip_cam_parallel_add:
    Middle Blk 使用 Strip 和 Channel Attention 的 full-width 并联注意力，
    两个分支最后直接相加融合。
    ...
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_Strip_CAM_Parallel_Add, self).__init__()
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_Strip_CAM_Parallel_Add(args)
    model.eval()
