...
def make_model(args, parent=False):
    return SymUNet_Pretrain_Strip_MHLA_Shift(args)
...
class MHLA2D(nn.Module):
    def __init__(self, dim, heads=4, window_size=49, transform="cos", shift_size=3):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x):
        """
        shift-only MHLA:
        1. reflect-pad 到 window 的整数倍
        2. 循环平移，让相邻窗口边界重新落入同一窗口
        3. 按原来的非重叠窗口做 MHLA
        4. shift 回来并裁掉 padding
        """
        B, C, H, W = x.shape
        ...
        x = x.view(B, pieces_h * pieces_w, self.window_size, C)
...
class MAB(nn.Module):
    def __init__(self, dim, num_head=2, kernel_sizes=[7, 11], dilations=[1, 1]):
        ...
        print(f"[MAB] 使用 shift-only MHLA2D (num_head={num_head}, shift=3)")
...
class SymUNet_Pretrain_Strip_MHLA_Shift(nn.Module):
    """
    symunet_pretrain_strip_mhla_shift: MAB使用仅带shift的MHLA替换NeighborhoodAttention
    ...
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_Strip_MHLA_Shift, self).__init__()
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_Strip_MHLA_Shift(args)
    model.eval()
