...
def make_model(args, parent=False):
    return SymUNet_Pretrain_LPU(args)
...
# ========== NAFBlock with LPU (Local Perception Unit) ==========
class NAFBlock(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        ...
        # LPU: Local Perception Unit - 3x3 depthwise convolution
        self.lpu = nn.Conv2d(in_channels=c, out_channels=c, kernel_size=3, padding=1, groups=c, bias=True)
...
#@ARCH_REGISTRY.register("CSymUNet_Pretrain_LPU")
class SymUNet_Pretrain_LPU(nn.Module):
    """
    预上采样版本SymUNet - LPU变种
    - 在NAFBlock中增加Local Perception Unit (LPU)
    """
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_LPU, self).__init__()
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_LPU(args)
    model.eval()
