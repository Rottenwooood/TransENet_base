...
def make_model(args, parent=False):
    return SymUNet_Pretrain_S1_Trans_NoMAB1_Conv(args)
...
class S1_TransBlock_NoMAB1_Conv(nn.Module):
    """
    S1 Series Block (去掉MAB1，用卷积替换MAB2):
    ...
    def __init__(self, c, drop_out_rate=0.):
        ...
        self.lk_mab = LargeKernelMAB(dim=c, kernel_sizes=[7, 11], dilations=[5, 3])
    ...
    def forward(self, x):
        ...
        # LAB -> LargeKernelMAB -> Conv (去掉MAB1，用卷积替换MAB2)
        x = self.lab(x)
        x = self.lk_mab(x)
        x = self.conv(x)
...
#@ARCH_REGISTRY.register("CSymUNet_Pretrain_S1_Trans_NoMAB1_Conv")
class SymUNet_Pretrain_S1_Trans_NoMAB1_Conv(nn.Module):
    """
    S1_Trans_NoMAB1_Conv: 去掉MAB1，用卷积替换MAB2
    - 使用 LAB (Local Aggregation Block)
    - 使用 MultiKernelConv (7x7, 11x11, dilations=[1,1])
    - 使用 MSConvStar
    """
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_S1_Trans_NoMAB1_Conv, self).__init__()
        ...
        for i, num in enumerate(enc_blk_nums):
            self.encoders.append(nn.Sequential(*[
                S1_TransBlock_NoMAB1_Conv(c=chan, drop_out_rate=drop_out_rate) for _ in range(num)
            ]))
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_S1_Trans_NoMAB1_Conv(args)
    model.eval()
