...
def trunc_normal_(*args, **kwargs):
    return nn.init.trunc_normal_(*args, **kwargs)
...
class HMoE(nn.Module):
    ...
    def forward(self, x: Tensor, x_t: Tensor):
        ...
        gate_scores, loss = self.gate(
            x_t, use_aux_loss=self.use_aux_loss
        )
...
class MoEUpsample(nn.Sequential):
    ...
    def __init__(self, scale, num_feat, num_experts):
        m = []
        m.append(HMoE(num_feat, scale * scale * num_feat, num_experts))
...
#     input = torch.rand(1, 3, 96, 96).cuda()  # B C H W
#     model = MFGHMOE().cuda()
