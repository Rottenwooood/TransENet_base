...
class HAA(nn.Module):
    """ Hierarchical Aggregation Attention"""
    def __init__(self, in_dim):
        super(HAA, self).__init__()
...
class HSFA(nn.Module):
    """ Hybrid Spatial Feature Aggregator
    Args:
...
class SFIIM(nn.Module):
    ...
    def __init__(self, in_nc):
        super(SFIIM, self).__init__()
        self.spatial_process = HSFA(in_nc, in_nc)  # 空间处理
        self.frequency_process = DFSE(in_nc)  # 频率处理
...
class SFMB(nn.Module):
    def __init__(
        ...
        self.self_attention = VSSM(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate, **kwargs)
...
class HAM(nn.Module):
    r""" HAM MModel
       ...
       Args:
           ...
           depths (tuple(int)): Depth of each SFMG
...
class SFMG(nn.Module):
    """Spatial–Frequency Mamba group (SFMG)
