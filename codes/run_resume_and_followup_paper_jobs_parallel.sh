...
RUN_TAG="${RUN_TAG:-paper_resume_followup_parallel_${FOLLOWUP_RUN_TS}}"
...
CGA_X3_SAVE_NAME="${CGA_X3_SAVE_NAME:-paper_cga_RSSCN7_x3_${FOLLOWUP_RUN_TS}}"
CGA_X3_RESUME="${CGA_X3_RESUME:-0}"
CGA_X4_SAVE_NAME="${CGA_X4_SAVE_NAME:-paper_cga_RSSCN7_x4_${FOLLOWUP_RUN_TS}}"
CGA_X4_RESUME="${CGA_X4_RESUME:-0}"
...
run_train_job() {
    ...
    echo "[train] gpu=${gpu_id} model=${model_name} scale=x${scale} save=${save_name} resume=${resume_flag}"
...
if [[ "${SKIP_STAGE1}" == "1" ]]; then
    echo "Skipping Stage 1 resume jobs because SKIP_STAGE1=1"
else
    # Stage 1: resume the two interrupted runs in parallel.
    run_train_job "${GPU0}" "cga_paper" "2" \
        "paper_cga_RSSCN7_x2_20260718_162858" "1" \
        "${RESULTS_DIR}/cga_rsscn7_x2_resume.txt" \
        "${LOG_DIR}/cga_rsscn7_x2_resume.log" &
    pid_cga_x2=$!

    run_train_job "${GPU1}" "mt_paper" "4" \
        "paper_mt_RSSCN7_x4_20260718_162858" "1" \
        "${RESULTS_DIR}/mt_rsscn7_x4_resume.txt" \
        "${LOG_DIR}/mt_rsscn7_x4_resume.log" &
    pid_mt_x4=$!

    if ! wait_all "${pid_cga_x2}" "${pid_mt_x4}"; then
        status=1
    fi
fi

# Stage 2: run the remaining cga RSSCN7 jobs in parallel.
run_train_job "${GPU0}" "cga_paper" "3" \
    "${CGA_X3_SAVE_NAME}" "${CGA_X3_RESUME}" \
    "${RESULTS_DIR}/cga_rsscn7_x3.txt" \
    "${LOG_DIR}/cga_rsscn7_x3.log" &
pid_cga_x3=$!

run_train_job "${GPU1}" "cga_paper" "4" \
    "${CGA_X4_SAVE_NAME}" "${CGA_X4_RESUME}" \
    "${RESULTS_DIR}/cga_rsscn7_x4.txt" \
    "${LOG_DIR}/cga_rsscn7_x4.log" &
pid_cga_x4=$!

if ! wait_all "${pid_cga_x3}" "${pid_cga_x4}"; then
    status=1
fi

# Stage 3: run cga AID -> WHU-RS19 test-only, two jobs at a time.
run_test_job "${GPU0}" "2" \
    "${RESULTS_DIR}/cga_aid_to_whu_rs19_x2.txt" \
    "${LOG_DIR}/cga_aid_to_whu_rs19_x2.log" &
pid_whu_x2=$!

run_test_job "${GPU1}" "3" \
    "${RESULTS_DIR}/cga_aid_to_whu_rs19_x3.txt" \
    "${LOG_DIR}/cga_aid_to_whu_rs19_x3.log" &
pid_whu_x3=$!

if ! wait_all "${pid_whu_x2}" "${pid_whu_x3}"; then
    status=1
fi

run_test_job "${GPU0}" "4" \
    "${RESULTS_DIR}/cga_aid_to_whu_rs19_x4.txt" \
    "${LOG_DIR}/cga_aid_to_whu_rs19_x4.log" || status=1
