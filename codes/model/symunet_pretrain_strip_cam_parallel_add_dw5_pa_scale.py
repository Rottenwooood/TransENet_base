...
def make_model(args, parent=False):
    return SymUNet_Pretrain_Strip_CAM_Parallel_Add_DW5_PA_SCALE(args)
...
class S1_TransBlock_NoMAB1(nn.Module):
    ...
    def forward(self, x):
        ...
        # LAB -> MAB2 + alpha * PA(shortcut) -> Conv (去掉MAB1)
        x = self.lab(x)
        x = self.mab2(x)
        x = x + self.pa_scale * pa_out
        x = self.conv(x)
...
#@ARCH_REGISTRY.register("symunet_pretrain_strip_cam_parallel_add_dw5_pa_scale")
class SymUNet_Pretrain_Strip_CAM_Parallel_Add_DW5_PA_SCALE(nn.Module):
    """
    symunet_pretrain_strip_cam_parallel_add_dw5_pa_scale:
    Middle Blk 使用:
    ...
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_Strip_CAM_Parallel_Add_DW5_PA_SCALE, self).__init__()
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_Strip_CAM_Parallel_Add_DW5_PA_SCALE(args)
    model.eval()
