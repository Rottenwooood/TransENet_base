...
def make_model(args, parent=False):
    return SymUNet_Pretrain_S1_Trans_Strip(args)
...
# ============== S1_Trans Block (NoMAB1 + Strip替换MA) ==============
class S1_TransBlock_Strip(nn.Module):
    """
    ...
    def forward(self, x):
        ...
        # LAB -> StripMAB -> Conv (去掉MAB1，用Strip替换MAB2)
        x = self.lab(x)
        x = self.strip_mab(x)
        x = self.conv(x)
...
class SymUNet_Pretrain_S1_Trans_Strip(nn.Module):
    ...
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_S1_Trans_Strip, self).__init__()
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_S1_Trans_Strip(args)
    model.eval()
