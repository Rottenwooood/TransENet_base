...
def make_model(args, parent=False):
    return SymUNet_Pretrain_Strip_MHLA_Only(args)
...
class MAB(nn.Module):
    def __init__(self, dim, num_head=2, kernel_sizes=[7, 11], dilations=[1, 1]):
        ...
        print(f"[MAB] 使用 MHLA-only 分支 (num_head={num_head}, overlap=2, shift=3)")
...
class SymUNet_Pretrain_Strip_MHLA_Only(nn.Module):
    """
    symunet_pretrain_strip_mhla_only:
    - 仅保留 MHLA 注意力分支
    - 移除 NAT 并联主分支
    ...
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_Strip_MHLA_Only, self).__init__()
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_Strip_MHLA_Only(args)
    model.eval()
