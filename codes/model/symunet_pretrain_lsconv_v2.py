...
def make_model(args, parent=False):
    return SymUNet_Pretrain_LSConv_v2(args)
...
class LSConvMiddleBlock(nn.Module):
    """
    Middle Block using LSConv_v2(无BN)
    LSConv_v2+ FFN
    """
...
#@ARCH_REGISTRY.register("symunet_pretrain_LSConv_v2")
class SymUNet_Pretrain_LSConv_v2(nn.Module):
    """
    symunet_pretrain_LSConv_v2: Middle Blk使用LSConv_v2(无BN)
    - 使用 LAB (Local Aggregation Block)
    - 仅使用 MAB2 (dilations=[5,3])
    - 使用 MSConvStar
    - Middle Blk使用LSConv_v2(去掉BatchNorm2d)
    """
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_LSConv_v2, self).__init__()
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_LSConv_v2(args)
    model.eval()
