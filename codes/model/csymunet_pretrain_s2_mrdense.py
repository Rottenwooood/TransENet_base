...
def make_model(args, parent=False):
    return SymUNet_Pretrain_S2_MRDense(args)
...
# ============== S2_MRDense Block (并联门控架构) ==============
class S2_MRDenseBlock(nn.Module):
    """
    S2 Series Block (并联门控架构):
    Token Mixing: x -> LayerNorm -> 1x1 Conv升维2C -> 劈两半 -> 分支A:3x3DWConv, 分支B:MRDenseDWConv -> 门控融合 -> SCA -> 降维 -> 残差
    ...
    def __init__(self, c, drop_out_rate=0.):
        ...
        # 分支B: MRDenseDWConv (组1:5x5, 组2:7x7, 组3:11x11)
        self.branch_b = MRDenseDWConv(dim=c)
...
class SymUNet_Pretrain_S2_MRDense(nn.Module):
    ...
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_S2_MRDense, self).__init__()
        ...
        for i, num in enumerate(enc_blk_nums):
            self.encoders.append(nn.Sequential(*[
                S2_MRDenseBlock(c=chan, drop_out_rate=drop_out_rate) for _ in range(num)
            ]))
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_S2_MRDense(args)
    model.eval()
