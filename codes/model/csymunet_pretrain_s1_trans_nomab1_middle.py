...
def make_model(args, parent=False):
    return SymUNet_Pretrain_S1_Trans_NoMAB1_Middle(args)
...
# ============== S1_Trans Block (NoMAB1 - dilations=[7,4]) ==============
class S1_TransBlock_NoMAB1_Middle(nn.Module):
    """
    S1 Series Block (去掉MAB1) - 用于Middle Block:
    x = LAB(x) -> MAB(dilations=[7,4]) -> Conv -> +shortcut
    """
    def __init__(self, c, drop_out_rate=0.):
        super().__init__()
        # LAB for local aggregation
...
class SymUNet_Pretrain_S1_Trans_NoMAB1_Middle(nn.Module):
    """
    S1_Trans_NoMAB1_Middle: Middle Block使用Decoder块(dilations=[7,4])
    - 使用 LAB (Local Aggregation Block)
    - 仅使用 MAB2 (dilations=[7,4])
    - 使用 MSConvStar
    - Middle Block使用Decoder块
    """
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_S1_Trans_NoMAB1_Middle, self).__init__()
