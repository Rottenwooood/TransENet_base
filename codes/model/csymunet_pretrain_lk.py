...
def make_model(args, parent=False):
    return SymUNet_Pretrain_LargeKernel(args)
...
# ========== Large Kernel NAFBlock for Bottleneck ==========
class NAFBlockLargeKernel(nn.Module):
    """
    大核NAFBlock - 用于bottleneck
    将3x3深度卷积替换为7x7或11x11
    """
    def __init__(self, c, kernel_size=7, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        super().__init__()
        dw_channel = c * DW_Expand

        # 大核深度卷积
        assert kernel_size in [7, 11], "kernel_size must be 7 or 11"
        padding = kernel_size // 2

        self.conv1 = nn.Conv2d(in_channels=c, out_channels=dw_channel, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        self.conv2 = nn.Conv2d(in_channels=dw_channel, out_channels=dw_channel, kernel_size=kernel_size, padding=padding, stride=1, groups=dw_channel,
                               bias=True)
        self.conv3 = nn.Conv2d(in_channels=dw_channel // 2, out_channels=c, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
...
#@ARCH_REGISTRY.register("CSymUNet_Pretrain_LargeKernel")
class SymUNet_Pretrain_LargeKernel(nn.Module):
    """
    预上采样版本SymUNet - LargeKernel变种
    - 将bottleneck中的TransformerBlock替换为大核NAFBlock
    - 保持NAFBlock结构，但将3x3 DWConv改为7x7或11x11
    """
    def __init__(self, args, conv=common.default_conv):
        super(SymUNet_Pretrain_LargeKernel, self).__init__()
        ...
        # 大核参数 (7 或 11)
        large_kernel_size = getattr(args, 'symunet_pretrain_large_kernel_size', 7)
        ...
        # 使用大核NAFBlock替换TransformerBlock作为bottleneck
...
if __name__ == "__main__":
    ...
    model = SymUNet_Pretrain_LargeKernel(args)
    model.eval()
